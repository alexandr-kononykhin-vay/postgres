...
// BitMaskAndNeq adds to filter bit mask condition
func BitMaskAndNeq(column string, val, exp interface{}) FnOpt {
	return func(opt *Opt) {
		opt.Filter = append(opt.Filter, filter.BitMaskAndNeq{
			Column:   column,
			Value:    val,
			Expected: exp,
		})
...
// BitMaskOrNeq adds to filter bit mask condition
func BitMaskOrNeq(column string, val, exp interface{}) FnOpt {
	return func(opt *Opt) {
		opt.Filter = append(opt.Filter, filter.BitMaskOrNeq{
			Column:   column,
			Value:    val,
			Expected: exp,
		})
...
// BitMaskAndEq adds to filter bit mask condition
func BitMaskAndEq(column string, val, exp interface{}) FnOpt {
	return func(opt *Opt) {
		opt.Filter = append(opt.Filter, filter.BitMaskAndEq{
			Column:   column,
			Value:    val,
			Expected: exp,
		})
...
// BitMaskOrEq adds to filter bit mask condition
func BitMaskOrEq(column string, val, exp interface{}) FnOpt {
	return func(opt *Opt) {
		opt.Filter = append(opt.Filter, filter.BitMaskOrEq{
			Column:   column,
			Value:    val,
			Expected: exp,
		})
...
// PostGISIntersectionWithCircle adds to filter to validate the PostGIS geometry object intersection with the circle
// at the point of Longitude, Latitude, and the given Radius in meters.
func PostGISIntersectionWithCircle(column string, longitude, latitude, radiusMeters float64) FnOpt {
	return func(opt *Opt) {
		opt.Filter = append(opt.Filter, filter.PostGISIntersectionWithCircle{
			Column:       column,
			Longitude:    longitude,
			Latitude:     latitude,
